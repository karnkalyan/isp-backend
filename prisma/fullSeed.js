...
async function main() {
    ...
    const isp = await prisma.iSP.create({
        data: {
            companyName: 'Kisan ISP',
            masterEmail: 'admin@kisan.net.np',
            businessType: 'ISP',
            website: 'https://kisan.net.np',
            contactPerson: 'Kisan Admin',
    ...
    const biratnagarBranch = await prisma.branch.create({
        data: { name: 'Biratnagar Branch', code: 'BR-BRT', email: 'biratnagar@kisan.net.np', ispId: isp.id, parentId: hq.id }
    });
    const itahariBranch = await prisma.branch.create({
        data: { name: 'Itahari Branch', code: 'BR-ITH', email: 'itahari@kisan.net.np', ispId: isp.id, parentId: hq.id }
    });
    const dharanBranch = await prisma.branch.create({
        data: { name: 'Dharan Branch', code: 'BR-DHR', email: 'dharan@kisan.net.np', ispId: isp.id, parentId: hq.id }
    });
    const butwalBranch = await prisma.branch.create({
        data: { name: 'Butwal Branch', code: 'BR-BTW', email: 'butwal@kisan.net.np', ispId: isp.id, parentId: hq.id }
    });
    const bharatpurBranch = await prisma.branch.create({
        data: { name: 'Bharatpur Branch', code: 'BR-BHP', email: 'bharatpur@kisan.net.np', ispId: isp.id, parentId: hq.id }
    });
    ...
    const usersToCreate = [
        ...
        { email: 'admin@kisan.net.np', name: 'Kisan Global Admin', role: adminRole, dept: itDept },
        { email: 'admin@kisan.com.np', name: 'Kisan Global Admin', role: adminRole, dept: itDept },
        { email: 'global.support@kisan.net.np', name: 'Global Support HQ', role: globalSupportRole, dept: supportDept },
        { email: 'global.tech@kisan.net.np', name: 'Global Tech HQ', role: globalTechnicalRole, dept: itDept },
        { email: 'global.marketing@kisan.net.np', name: 'Global Marketing HQ', role: globalMarketingRole, dept: marketingDept },
        { email: 'global.field@kisan.net.np', name: 'Global Field HQ', role: globalFieldRole, dept: supportDept },

        // Head Office (HQ) Branch Users
        { email: 'hq.admin@kisan.net.np', name: 'HQ Branch Admin', role: branchAdminRole, branch: hq, dept: itDept },
        { email: 'hq.support@kisan.net.np', name: 'HQ Support Agent', role: branchSupportRole, branch: hq, dept: supportDept },
        { email: 'hq.field@kisan.net.np', name: 'HQ Field Tech', role: branchFieldRole, branch: hq, dept: supportDept },
        { email: 'hq.marketing@kisan.net.np', name: 'HQ Marketing Agent', role: branchMarketingRole, branch: hq, dept: marketingDept },
        { email: 'hq.tech@kisan.net.np', name: 'HQ Network Tech', role: branchTechnicalRole, branch: hq, dept: itDept },

        // Pokhara Branch Users
        { email: 'pkr.admin@kisan.net.np', name: 'Pokhara Branch Admin', role: branchAdminRole, branch: pkrBranch, dept: itDept },
        { email: 'pkr.support@kisan.net.np', name: 'Pokhara Support Agent', role: branchSupportRole, branch: pkrBranch, dept: supportDept },
        { email: 'pkr.field@kisan.net.np', name: 'Pokhara Field Tech', role: branchFieldRole, branch: pkrBranch, dept: supportDept },
        { email: 'pkr.marketing@kisan.net.np', name: 'Pokhara Marketing', role: branchMarketingRole, branch: pkrBranch, dept: marketingDept },
        { email: 'pkr.tech@kisan.net.np', name: 'Pokhara NOC', role: branchTechnicalRole, branch: pkrBranch, dept: itDept },

        // Customer
        { email: 'customer@kisan.net.np', name: 'Ram Bahadur', role: customerRole, branch: hq, dept: null },

        // Multi-Branch User (Regional Manager - Western)
        {
            email: 'regional@kisan.net.np',
    ...
    const superAdmin = await prisma.user.findUnique({ where: { email: 'admin@kisan.net.np' } });
    const staffUser = await prisma.user.findUnique({ where: { email: 'hq.field@kisan.net.np' } });
