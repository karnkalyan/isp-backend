...
async function main() {
    ...
    const isp = await prisma.iSP.create({
        data: {
            ...
            masterEmail: 'admin@ispmanager.com',
            businessType: 'ISP',
    ...
    // --- 5. Seed Departments ---
    const departments = {};
    for (const name of ['IT', 'Support', 'Sales', 'Marketing', 'Technical', 'Billing']) {
    ...
    const usersToCreate = [
        { email: 'admin@ispmanager.com', name: 'ISP Manager Admin', role: adminRole, department: departments.IT },
    ];
