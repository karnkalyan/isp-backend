...
async function assignInventoryItem(req, res, next) {
    try {
        ...
        if (customerId && !req.customerProvisioningAssignment) {
            return res.status(400).json({
                error: 'Inventory assignment is for internal staff only. Assign customer hardware from the customer provisioning workflow.'
            });
        }
