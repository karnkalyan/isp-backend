...
app.get('/api/nas-devices', isAuthenticated(prisma), checkPermission('nas_read'), customerController.listNasDevices);
app.get('/api/sessions', isAuthenticated(prisma), checkPermission('customer_read'), customerController.listActiveSessions);
app.get('/api/sessions/:username', isAuthenticated(prisma), checkPermission('customer_read'), customerController.getSessionInfoForUser);
app.post('/api/disconnect/:username', isAuthenticated(prisma), checkPermission('customer_update'), customerController.disconnectLatestSession);
app.post('/api/disconnect/:username/all', isAuthenticated(prisma), checkPermission('customer_update'), customerController.disconnectAllSessions);
app.post('/api/disconnect/session/:sessionId', isAuthenticated(prisma), checkPermission('customer_update'), customerController.disconnectBySessionId);
